(function (global) {
  ...
  var CATALOG_ROLE_SCORE = { core:30, supporting:14, excluded:-999 };
  var RESULT_PRIORITY_SCORE = { primary:24, alternative:10, conditional:0, hidden:-999 };
  ...
  function buildProfile(input) {
    ...
    if (has(a,'q1','트러블 — 자꾸 뭐가 올라와요')) concerns.push('acne','soothing');
    if (one(a,'q4') === '블랙헤드·좁쌀 — 피부 아래 오돌토돌') concerns.unshift('blackheads','pores','comedonal_acne');
    if (one(a,'q4') === '빨갛게 올라왔다 가라앉는 것') concerns.unshift('inflammatory_acne','soothing');
    ...
    if (level === 'beginner' && !f.chronic_sensitive && !f.emergency_state && includesAny(concerns,['blackheads','pores'])) maxStrength = 'moderate';
    ...
    profile.parallelHydration = profile.hydrationLow && includesAny(profile.concerns,['pores','blackheads','pigmentation','brightening','dullness','aging','wrinkles']);
    return profile;
  }

  function eligible(p, profile) {
    if (!p || p.catalog_role === 'excluded' || p.result_priority === 'hidden' || p.access_level === 'blocked' || p.data_status !== 'verified') return false;
    if (p.access_level === 'cautious' && p.slot === 'active' && profile.activeToleranceLevel !== 'experienced') return false;
    ...
    if (p.slot === 'active') {
      ...
      if (profile.skinState === 'chronically_sensitive' && profile.deferredConcerns.length) {
        ...
        var currentSafe=includesAny(targets,['soothing','mild_exfoliation','pores','blackheads','comedonal_acne','clogged_pores','sebum']);
  ...
  function score(p, profile, role) {
    var s = (CATALOG_ROLE_SCORE[p.catalog_role] || 0) + (RESULT_PRIORITY_SCORE[p.result_priority] || 0);
    if (p.access_level === 'cautious') s -= 8;
    if (p.slot === role) s += 40;
    ...
    if (p.slot === 'active') {
      ...
      if (includesAny(profile.concerns,['blackheads','pores','sebum_control']) && types.indexOf('bha') !== -1) s += 38;
  ...
  function concernFocus(c) {
    if (includesAny([c],['blackheads','pores','comedonal_acne','sebum_control'])) return 'pores';
  ...
  function activeRelevant(p, profile, selectedFocus) {
    ...
    if (focus==='pores' && (includesAny(t,['blackheads','comedonal_acne','clogged_pores','pores','sebum','mild_exfoliation']) || a.indexOf('bha')!==-1)) return true;
    ...
    if (focus==='pigmentation' && includesAny(t,['melasma','post_inflammatory_hyperpigmentation','dark_spots','dullness','pigmentation','brightening'])) return true;
  ...
  function label(p, profile, selectedFocus) {
    ...
    return focus==='pores' ? '모공·블랙헤드 관리' : focus==='acne' ? '트러블 진정 관리' :
      focus==='pigmentation' ? '잡티·색소 관리' : focus==='redness' ? (profile.concerns.indexOf('pie')!==-1 ? '붉은 자국·결 진정 관리' : '붉어짐 진정 관리') :
  ...
  function userReason(p, profile, selectedFocus) {
    ...
    if (p.product_type==='cleansing_oil') return '선크림과 메이크업을 부드럽게 녹이면서 향료는 뺀 제품이에요. 저녁 첫 세안으로 잘 맞아요.';
    if (p.product_type==='cleansing_milk') return '피부가 쉽게 당길 때도 부드럽게 닦여요. 건조한 피부의 첫 세안으로 추천해요.';
    if (p.product_type==='micellar_water') return '향료 없이 가볍게 닦이는 제품이라, 예민했던 경험이 있는 피부도 부담을 줄이기 좋아요.';
    if (p.product_type==='foam_cleanser') return profile.oilLevel==='high' ? '번들거림은 씻어내면서도 피부가 뻣뻣해지지 않아요. 매일 쓰기 편한 세안제예요.' : '씻고 난 뒤 당김이 적고, 매일 써도 부담 없는 순한 세안제예요.';
    if (p.slot==='sunscreen') {
      if (profile.finish==='toneup_cover') return '자외선을 막으면서 붉은기와 칙칙한 톤까지 자연스럽게 가려줘요.';
      if (profile.finish==='toneup_light') return '자외선을 차단하면서 얼굴빛도 가볍게 밝혀주는 제품이에요.';
      if (profile.finish==='base') return '들뜨지 않고 촉촉하게 밀착돼서 화장 전 베이스로 바르기 편해요.';
      if (profile.noOrganic) return '눈시림이 걱정된다고 해서 유기자차 성분을 빼고 고른 무기자차예요.';
      return '매일 부담 없이 쓸 수 있는 사용감과 충분한 자외선 차단력을 같이 갖췄어요.';
    }
    if (p.slot==='moisturizer') {
      if (profile.oilLevel==='high'||profile.oilLevel==='mixed_high') return '유분은 이미 충분한 편이라, 무겁게 덮기보다 수분을 채우고 가볍게 마무리돼요.';
      if (profile.skinState!=='stable'||includesAny(target,['barrier_repair','soothing'])) return '예민해지기 쉬운 피부를 편안하게 감싸고, 수분이 빠져나가지 않게 잡아줘요.';
      return '건조함을 오래 잡아주면서 발랐을 때 편안하게 느껴지는 보습제예요.';
    }
    if (p.slot==='skin') return includesAny(target,['soothing','redness']) ? '붉고 예민해진 피부를 먼저 편안하게 달래는 데 잘 맞아요.' : '세안 뒤 부족한 수분을 가볍게 채워줘서 다음 제품이 편하게 발려요.';
    if (a.indexOf('bha')!==-1) return '모공 속 피지와 묵은 각질을 정리하는 성분이 들어 있어, 블랙헤드·막힌 모공 관리에 도움이 돼요.';
    if (a.indexOf('pha')!==-1) return '강한 각질 성분보다 순하게 피부결을 정돈해줘서, 자극이 걱정될 때 택했어요.';
    if (includesAny(a,['txa','vitamin_c'])) return '갈색 자국과 칙칙한 피부톤을 함께 관리할 수 있는 성분 구성이에요.';
    if (a.indexOf('alpha_arbutin')!==-1) return '자극을 느꼈던 미백 성분은 빼고, 알파 알부틴으로 갈색 자국을 관리하는 대체 제품이에요.';
    if (a.indexOf('retinoid')!==-1) return '탄력과 잔주름을 함께 관리할 수 있고, 지금까지의 사용 경험에 맞는 강도예요.';
    if (a.indexOf('bakuchiol')!==-1) return '탄력은 관리하고 싶지만 레티놀 자극은 피하고 싶을 때 쓰기 좋은 대안이에요.';
    if (focus==='redness'||includesAny(target,['soothing','redness'])) return '지금은 강한 기능성보다 붉어짐을 편안하게 달래는 게 먼저라 이 제품을 택했어요.';
    return '지금 피부 고민과 원하는 사용감에 가장 잘 맞는 제품이에요.';
  }
  function toSlot(hit, profile, selectedFocus) {
    ...
    if (profile.skinState==='chronically_sensitive' && p.product_name==='에스트라 에이시카365 흔적진정세럼') {
      reason='미백은 다음 단계로 미루고, 지금은 진정과 순한 PHA 결 정돈으로 붉어짐·막힌 모공을 함께 관리해요.';
    }
    var priority=p.slot==='moisturizer'||p.slot==='sunscreen'?'essential':(p.slot==='active'?'concern':'optional');
    return { label:label(p,profile,selectedFocus), slotType:p.slot, name:p.product_name, price:p.price_krw || 0, why:reason, use:useText, actives:p.active_types || [], coupangUrl:'', tier:p.recommendation_status, priority:priority, matchScore:hit.score,
      catalogRole:p.catalog_role || 'supporting', accessLevel:p.access_level || 'standard', resultPriority:p.result_priority || 'alternative',
  ...
  function buildRoutine(profile) {
    ...
    var coreNote = profile.routineMode==='minimal' ? '지금은 진정·보습·자외선 차단만 유지하세요.'
      : profile.parallelHydration ? '수분 보완을 계속하면서 모공·톤 같은 고민 관리도 한 가지씩 함께 시작해요.'
