(function (global) {
  ...
  function eligible(p, profile) {
    ...
    if ((p.access_level === 'cautious' || p.recommendation_status === 'gated') && profile.activeToleranceLevel !== 'experienced') return false;
  ...
  function score(p, profile, role) {
    var priorityTier = { primary:30, alternative:18, conditional:8, hidden:-999 };
    var s = priorityTier[p.result_priority] != null ? priorityTier[p.result_priority] : (TIER[p.recommendation_status] || 0);
    if (p.slot === role) s += 40;
  ...
  function userReason(p, profile, selectedFocus) {
    ...
    if (a.indexOf('bha')!==-1) return '모공 속 피지와 묵은 각질을 정리하는 성분이 들어 있어, 블랙헤드와 막힌 모공 관리에 골랐어요.';
  ...
  function toSlot(hit, profile, selectedFocus) {
    ...
    return { label:label(p,profile,selectedFocus), slotType:p.slot, name:p.product_name, price:p.price_krw || 0, why:reason, use:useText, actives:p.active_types || [], coupangUrl:'', tier:p.result_priority || p.recommendation_status, catalogRole:p.catalog_role || '', accessLevel:p.access_level || '', priority:priority, matchScore:hit.score,
