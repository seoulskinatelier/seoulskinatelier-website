(function (global) {
  ...
  function eligible(p, profile) {
    if (!p || p.recommendation_status === 'excluded' || p.data_status !== 'verified') return false;
    if (p.recommendation_status === 'gated' && profile.activeToleranceLevel !== 'experienced') return false;
  ...
  function score(p, profile, role) {
    var s = TIER[p.recommendation_status] || 0;
    if (p.slot === role) s += 40;
