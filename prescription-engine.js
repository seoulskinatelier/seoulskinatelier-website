(function (global) {
  ...
  function activeRelevant(p, profile, selectedFocus) {
    ...
    if (focus==='pigmentation' && (includesAny(t,['melasma','post_inflammatory_hyperpigmentation','dark_spots','dullness']) || includesAny(a,['txa','vitamin_c','alpha_arbutin']))) return true;
  ...
  function label(p, profile, selectedFocus) {
    ...
    return focus==='pores' ? '모공·블랙헤드 관리' : focus==='acne' ? '트러블 진정 관리' :
      focus==='pigmentation' ? '잡티·색소 관리' : focus==='redness' ? '붉어짐 진정 관리' :
